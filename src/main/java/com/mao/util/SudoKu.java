...
public class SudoKu {
     ...
     * @param y y竖
     */
    public void jx(int[][] src, int x, int y){
        if (src[x][y] == 0){
            int[][] copy = copy(src);
     ...
     * @return [x,y]
     */
    public int[] nextPosition(int x, int y){
        if (x == 8)
            if (y == 8) return null;
     ...
     * @return 可能值
     */
    public int[] getProbability(int[][] src, int x, int y){
